...
** josh on client and RP sending proofs to IdP [[gnus:nnimap%2Badbc:INBOX#CF180C59.D947%25josh.howlett@ja.net][Email from Josh Howlett: Re: ABFAB ephemeral keys chat]]
...
[FIXME]
**** EAP exposed between the initiator and acceptor
Access to the MSK/EMSK gives the possibility to decrypt [and tamper
with?] the traffic protected by the EAP tunnel, i.e. traffic between
...
- Key material
  ...
  [this is where MSK+EMSK are derived by the IdP (server) and sent to
...
establishment and use it to encrypt other subtokens.
